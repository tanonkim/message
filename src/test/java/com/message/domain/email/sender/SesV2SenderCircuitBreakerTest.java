...
class SesV2SenderCircuitBreakerTest {
    ...
    @Mock private SesProperties sesProperties;
    @Mock private StageEmailFilter stageEmailFilter;

    private SesV2Sender sesV2Sender;

    private static final NotificationMessage 테스트_메시지 = new NotificationMessage(
            1L, "EMAIL", "NORMAL", "user@example.com",
            null, null, "주문 확인 이메일입니다.", null, null, 0
    );

    @Test
    @DisplayName("서킷 OPEN 시 fallback이 호출되어 failure 반환")
    void 서킷_오픈_시_fallback_failure_반환() {
        given(sesProperties.region()).willReturn("ap-northeast-2");
        sesV2Sender = new SesV2Sender(sesProperties, stageEmailFilter);

        CallNotPermittedException 서킷오픈_예외 = CallNotPermittedException.createCallNotPermittedException(
                CircuitBreaker.of("ses", CircuitBreakerConfig.ofDefaults())
        );

        SendResult result = sesV2Sender.fallback(테스트_메시지, 서킷오픈_예외);

        assertThat(result.success()).isFalse();
        assertThat(result.errorMessage()).isEqualTo("Circuit Breaker OPEN: 이메일 서비스 일시 중단");
